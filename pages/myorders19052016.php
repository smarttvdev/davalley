...
if(isset($_SESSION['tipsval'])){ 
$tipsval = number_format($_SESSION['tipsval'],2) ;
}else{
$tipsval = number_format(0,2);
}

if(isset($_SESSION['tenderval'])){ 
$tenderval = number_format($_SESSION['tenderval'],2) ;
}else{
$tenderval = number_format(0,2);
}
...
<div class="popup-div clearfix" id="paycash-dialog" style="display:none;">
	...
	<div class="popup-content clearfix">
		...
		<input name="txtpaycash" id="txtpaycash" type="text" class="popup-input" value="<?php echo $tenderval;?>">
...
<table border="0" style="width:90%; border:none !important;">
...
<div class="popup-div clearfix" id="tips-dialog" style="display:none;">
	...
	<div class="popup-content clearfix">
		...
		<input name="txttips" id="txttips" type="text" class="popup-input" value="<?php echo $tipsval;?>">
	...
	<input type="hidden" id="tips-hidden" value="">
		...
		<input name="tips_submit" type="button" id="tips-submit" class="popup-submit" value="Submit" onclick="tipsSubmit();">
	</div>
	</form>
</div>
...
echo '<tr id="tipsshowcart" ><td></td><td></td><td></td><td></td><td><b>Tips:</b></td><td>$'.$tipsval.'</td></tr>';
echo '<tr id="finaltotalshowcart" ><td></td><td></td><td></td><td></td><td><b>Total:</b></td><td>$'.$total.'</td></tr>';
echo '<tr id="tendershowcart" ><td></td><td></td><td></td><td></td><td><b>Tender:</b></td><td>$'.$tenderval.'</td></tr>';
echo '<tr id="changeshowcart" ></tr></tbody>';
}
...
if($_SESSION['UserLogin'] == 'TRUE' && $_SESSION['confirm_payment'] <> 1){ 
echo "<td><form id='confirmcheckOutSubmit' method='post' action='pages/myorders_addon.php?' >
<input type='submit' name='confirmsubmitbtn' class='button button2' value='Confirm Checkout'>
<input id='card_tips' type='hidden' name='indoortips' value='0'>
<input id='card_tender' type='hidden' name='indoortender' value='0'>
...
<form id='checkOutSubmit' method='post' action='".$url."' >
	...
	<input id='total_amount' type='hidden' name='x_amount' value='".$amount."' />
...
echo "
	<INPUT  TYPE='HIDDEN' name='x_line_item' VALUE='0<|> Tips<|> <|> 1 <|>".$tipsval."<|>N'> 

	<INPUT TYPE='HIDDEN' name='x_tax' VALUE=' Tax <|>".$total_tax."<|>".$total_tax."'>";
	
	
	

echo "<input type='button' id='checkoutbtn' class='button button2' value='".$label."'>";
...
echo "<td>	<input type='button' class='button button3' id='emptycartbtn' onClick='emptyCart();' value='Empty cart'></td></tr></tbody>"; 
?>
...
function paycashSubmit()
{
	...
	var finaltipsval= parseFloat(finaltipsval)
	...
	$('#card_tips').val(finaltipsval);
	$('#card_tender').val(tenderval);
...
function tipsSubmit()
{
	...
	var trTenderval = '<td></td><td></td><td></td><td></td><td><b>Tender:</b></td><td>$'+tenderval+'</td>';
	var trChangeval = '<td></td><td></td><td></td><td></td><td><b>Change:</b></td><td>$'+changeval+'</td>';
	$('#card_tips').val(tipsval);
	$('#card_tender').val(tenderval);
	//$('#total_amount').val(tipstotalval);
