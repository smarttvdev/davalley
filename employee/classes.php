...
HAVING SUM(  `quantity` )  LIMIT 0 , $limit");
	...
	function create_new_order($table_number){
		$insert = mysql_query("insert into apx_order(user_id,table_id,order_status,added_date,added_time) values('$_SESSION[user_id]','$table_number','Pending',NOW(),NOW())");
		if($insert)
		{ 
			$last_id = MySql_Insert_Id();
			$num = sprintf("%'.05d\n", $last_id);			
			mysql_query("update apx_order set ticket_id='$num' where id='$last_id'");
			return $last_id;
		}
	}
	function get_current_order_detail(){
		return $query = mysql_fetch_array(mysql_query("SELECT * FROM `apx_order` WHERE id='$_SESSION[order_id]'"));
	}
	function add_item_to_order($item_id,$form_data){
		...
		if($check_item<=0){
			...
			$old_trable_order = mysql_query("insert into orders(neworderID,itemID,cookWith,sideOrder,quantity,dateOrdered) values('$_SESSION[order_id]','$item_id','$all_side_orders','$sider_order','1',NOW())");
			$old_ordertableID = mysql_insert_id();
			$old_trable_order_paid = mysql_query("insert into ordersPaid(neworderID,orderID,itemID,cookWith,sideOrder,quantity,dateOrdered,cash,orderRead,orderCompleted,orderBy,price,userID,tableNum,GuestNum) values('$_SESSION[order_id]','$old_ordertableID','$item_id','$all_side_orders','$sider_order','1',NOW(),'0','1','','0','','','','')")or die(mysql_error());
...
function get_order_item_list_checkout($o_id = ""){
	...
	function update_item_qty($item_id,$qty){
		...
		$query = mysql_query("UPDATE orders SET quantity='$qty' WHERE itemID='$item_id' and neworderID='$_SESSION[order_id]'");
		$query = mysql_query("UPDATE ordersPaid SET quantity='$qty' WHERE itemID='$item_id' and neworderID='$_SESSION[order_id]'");
	}
	...
	function remove_item_from_order($id){
		...
		$query = mysql_query("DELETE FROM ordersPaid WHERE itemID='$id' and neworderID='$_SESSION[order_id]'");
		$query = mysql_query("DELETE FROM orders WHERE itemID='$id' and neworderID='$_SESSION[order_id]'");
	...
	function get_today_orders(){
		return $query = mysql_query("SELECT * FROM apx_order WHERE `added_date` >= DATE(NOW()) and user_id='$_SESSION[user_id]'");
	}
	function edit_order_from_front($order_id){
		$query = mysql_fetch_array(mysql_query("SELECT * FROM apx_order WHERE order_id='$order_id'"));
	...
	function save_order_payment($form_data){
		...
		if($check_payment<=0){
			...
			if($query){
				$update_order = mysql_query("UPDATE apx_order SET order_status='complete' WHERE id='$_SESSION[order_id]'");
				echo "1";
			}else{
				echo "0";
			}
	...
	function update_cart_payment($order_id, $card_amount){
		...
		if($remin_amt['cash_change']>=0){
			$update_order = mysql_query("UPDATE apx_order SET order_status='complete' WHERE id='$order_id'");
		}
